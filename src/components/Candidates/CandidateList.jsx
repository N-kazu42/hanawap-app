...
export default function CandidateList() {
  ...
  async function handleVote(candidateId, vote) {
    ...
    const next    = current === vote ? null : vote   // 同じボタンで取消
    ...
    <div className="pb-24">
      ...
      <div className="px-3 pt-4 pb-2 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-400">いつでも変更できます</p>
        </div>
        <button
      ...
      {candidates.length > 0 && (
        <div className="px-3 mb-3">
          <p className="text-xs text-gray-500 mb-1.5 font-medium">あなたは？</p>
            ...
            {MEMBERS.map(m => (
              <button
                ...
                className={`flex-1 py-2.5 rounded-xl text-sm font-bold transition-all
                            ${activeMember === m.id ? 'text-white shadow-md scale-105' : 'bg-white text-gray-400 shadow-sm'}`}
      ...
      {candidates.length === 0 && !showForm ? (
        <div className="text-center py-14 text-gray-400 px-3">
          ...
          <p className="text-sm">候補日がありません</p>
          <p className="text-xs mt-1">「候補日追加」で登録しましょう</p>
        </div>
      ) : candidates.length > 0 && (
        <div className="mx-3 bg-white rounded-2xl shadow-sm overflow-hidden">
          {/* テーブルヘッダー */}
          <div className="grid items-center bg-gray-50 border-b border-gray-100 px-3 py-2"
                ...
                <div className="grid items-center px-3 py-3"
                  ...
                  <div>
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <span className={`text-sm font-bold
                        ${isRed ? 'text-red-500' : isSat ? 'text-blue-500' : 'text-gray-800'}`}>
                        {label}
                      </span>
                      {allOk && (
                        ...
                        return (
                          <div key={m.id} className="flex items-center gap-0.5">
                            ...
                            <span className={`text-xs font-bold
                              ${v === '○' ? 'text-green-500' : v === '△' ? 'text-yellow-500' : v === '×' ? 'text-red-400' : 'text-gray-200'}`}>
                              {v || '–'}
                            </span>
                          </div>
                        )
                  ...
                  {VOTE_OPTIONS.map(v => (
                    <button
                      ...
                      disabled={isLoading}
                      className={`w-9 h-9 mx-0.5 rounded-xl text-sm font-bold border-2 transition-all active:scale-90
                                  ${currentVote === v ? VOTE_STYLE[v].active : VOTE_STYLE[v].inactive}`}
                    >
                      {v}
                    </button>
                  ))}
          ...
          {candidates.map(c => {
            ...
            return (
              <button
                ...
                className="flex items-center gap-1 text-xs text-gray-400 bg-white
                           px-2.5 py-1.5 rounded-full shadow-sm
